...
class huobi:
    ...
    def getOrderInfo(self,coinType, id, method="order_info"):
        ...
        if result and result['status'] == 'ok':
            ...
            if result['state'] == 'filled':
                ...
                orderInfo["status"] = "partial-filled"
    ...
    def query_withdraw(self, currency, start_id="", size=1):
        result = nh.query_withdraw(currency, start_id=id, size=size, account=self.account)
        if result and result["status"] == "ok":
            return {"result": "success", "msg": result["data"]}
        return {"result": "fail", "msg": result}
